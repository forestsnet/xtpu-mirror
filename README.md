> ### ℹ️ This is a mirror
>
> Upstream [`Mon-ius/XTPU`](https://github.com/Mon-ius/XTPU) was deleted from GitHub, which broke
> `bit.ly/create-cloudflare-warp` — it redirected to
> `raw.githubusercontent.com/Mon-ius/XTPU/refs/heads/main/cloudflare/create-cloudflare-warp.sh`,
> which now returns HTTP 404. Several projects fetched that URL at runtime and pipe it into `sh`,
> including [`forestsnet/xray-warp`](https://github.com/forestsnet/xray-warp).
>
> Recovered from the [`Cat-Ling/XTPU`](https://github.com/Cat-Ling/XTPU) snapshot (2025-11-10).
> All code is by the original author, **Monius**, and stays under **GPL-3.0**; the only change in
> this mirror is this banner.
>
> Git history is intentionally not mirrored: an early upstream commit contains a Hugging Face access
> token that was later replaced with a placeholder, and republishing that history would re-expose it.
> This repo is the file tree as of the snapshot. History remains visible at `Cat-Ling/XTPU`.
>
> Stable raw URL for the WARP registration script:
>
> ```
> https://raw.githubusercontent.com/forestsnet/xtpu-mirror/main/cloudflare/create-cloudflare-warp.sh
> ```
